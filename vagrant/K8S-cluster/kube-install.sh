...
function install_metrics_server(){
  ...
  {
    # Use online manifest if local file doesn't exist
    if [ -f manifests/metrics-server.yaml ]; then
      kubectl apply -f manifests/metrics-server.yaml
    else
      echo "Local metrics-server manifest not found, using online version"
      kubectl apply -f https://github.com/kubernetes-sigs/metrics-server/releases/latest/download/components.yaml
      # Patch metrics-server to work with self-signed certificates (common in dev environments)
...
function kubeadm_init(){
  ...
  {
    # Get the IP address of the private network interface (192.168.56.x for Vagrant)
    # First try to get IP from 192.168.56.0/24 network (Vagrant private network)
    MAIN_IP=$(ip -4 addr show | grep -oP '(?<=inet\s)192\.168\.56\.\d+' | head -1)
    
    # If not found, fall back to the default route interface
    if [ -z "$MAIN_IP" ]; then
